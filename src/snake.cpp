...
void Snake::Update() {
    SDL_Point prev_cell{
        ...
        static_cast<int>(head_y)};  // Lưu tọa độ của đầu rắn trước khi cập nhật.
    UpdateHead();
    SDL_Point current_cell{
        static_cast<int>(head_x),
        static_cast<int>(head_y)};  // Lưu tọa độ của đầu rắn sau khi cập nhật.

    // Cập nhật toàn bộ các phần thân rắn nếu đầu rắn đã di chuyển.
...
void Snake::UpdateHead() {
    ...
    // Kiểm tra các điều kiện để xác định nếu đầu rắn cần phải đổi hướng.
    switch (direction) {
    case Direction::kUp:
...
void Snake::UpdateBody(SDL_Point &current_head_cell, SDL_Point &prev_head_cell) {
    // Thêm vị trí của đầu rắn trước khi di chuyển vào vector thân rắn
    body.push_back(prev_head_cell);

    if (!growing) {
        // Xóa đuôi nếu không đang phát triển
    ...
    // Kiểm tra xem rắn có chết hay không (va vào thân mình).
...
Snake::Snake(int grid_width, int grid_height)
    ...
    // Thiết lập chiều dài ban đầu của rắn
    int x_incr, y_incr;
    switch (direction) {
    case Direction::kUp:
        x_incr = 0;
...
void Snake::ChangeDirection(Direction inpDir) {
    ...
    // Kiểm tra xem nếu cell tiếp theo đã bị chiếm bởi rắn thì không cho phép thay đổi hướng.
    if (SnakeCell(next_xy[0], next_xy[1]))
        direction = curDir;
    else {
        // Kiểm tra nếu cell tiếp theo ra ngoài phạm vi của grid thì không thay đổi hướng.
